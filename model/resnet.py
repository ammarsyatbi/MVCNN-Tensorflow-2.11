...
class ResNet(tf.keras.Model):
    def __init__(self, num_classes=1000):
        ...
        # Base Model
        self.features = tf.keras.applications.resnet50.ResNet50(
            include_top=False,
            weights='imagenet',
            pooling=None,
    ...
    def call(self, x):
        ...
        for i in range(int(cfg.views)):
            ...
            # Resnet Input preprocess
            v = tf.keras.applications.resnet50.preprocess_input(v, data_format=None)
            v = self.features(v)
            v = self.flatter(v)
...
def resnet(pretrained=False, **kwargs):
    ...
    print("Model initiated - ResNet50")
    return model
